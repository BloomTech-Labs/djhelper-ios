...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene,
        ...
        guard let url = connectionOptions.urlContexts.first?.url, let scheme = url.scheme else {
            print("App opened by user and not by clicking on a link: \(#line) in function: \(#function)\n")
            return
        }
        
        print("URL (from clicking a link): \(url) and the associated scheme: \(scheme)")
        
        let urlComponents = URLComponents(url: url, resolvingAgainstBaseURL: true)
        //        let tempUrlToPass = URL(string: "djscheme://www.djhelper.com/guestLogin?eventId=\(1)")!
        
        let eventID = urlComponents?.queryItems?.first?.value
        print("djID = \(String(describing: eventID))")
        
        if scheme == "djscheme" {
            let storyboard = UIStoryboard(name: "Main", bundle: .main)
            let guestLoginVC = storyboard.instantiateViewController(withIdentifier: "guestLoginVC")
